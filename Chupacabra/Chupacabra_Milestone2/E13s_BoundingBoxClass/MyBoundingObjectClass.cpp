...
MyBoundingObjectClass::MyBoundingObjectClass(std::vector<vector3> listOfVertex, bool isSphere)
{
	if (!isSphere)
	{
		std::cout << "wtf";
	...
	// find farthest distance in sphere
	//Count the points of the incoming list
	uint nVertexCount = listOfVertex.size();
	...
	float fMax = listOfVertex[0].x;
	float fMin = listOfVertex[0].x;
	for (uint nVertex = 1; nVertex < nVertexCount; nVertex++)
	{
		//if (m_v3Min.x > listOfVertex[nVertex].x) //If min is larger than current
		//	m_v3Min.x = listOfVertex[nVertex].x;
		if (fMin > listOfVertex[nVertex].x) //If min is larger than current
			fMin = listOfVertex[nVertex].x;
		if (fMax < listOfVertex[nVertex].x)//if max is smaller than current
			fMax = listOfVertex[nVertex].x;
